...
import { searchDeck, drawCards, returnToDeckTop, returnToDeckBottom, takeSideCard, takeSideRandom, shuffleDeck } from "./domain/card-actions";
...
import { promoteToBattle, needsBenchSelector } from "./domain/battle-bench-swap";
...
type Action =
  ...
  | { type: "MOVE_WITH_CLEANUP"; instanceId: string; from: ZoneName; to: ZoneName }
  | { type: "TRASH_RETURN"; instanceId: string; to: "手札" | "山札上" | "山札下" };

function withHistory(state: AppState, game: GameState): AppState {
  return { ...state, game, history: [...state.history, takeSnapshot(state.game)], warning: null };
}
...
function appReducer(state: AppState, action: Action): AppState {
  switch (action.type) {
    ...
    case "UNDO": {
      ...
      return { ...state, game: restoreSnapshot(prev), history: state.history.slice(0, -1) };
    }

    case "RESET":
      return { ...state, game: resetGame(state.game), history: [], modal: null, contextMenu: null };
    ...
    case "MOVE_CARD":
      return withHistory(state, moveCard(state.game, action.instanceId, action.from, action.to));

    case "MOVE_WITH_CLEANUP":
      return withHistory(state, moveCardWithCleanup(state.game, action.instanceId, action.from, action.to));

    case "SEARCH_SELECT":
      return { ...withHistory(state, searchDeck(state.game, action.selectedIds)), modal: null };
    ...
    case "RETURN_TOP":
      return { ...withHistory(state, returnToDeckTop(state.game, action.instanceId)), contextMenu: null };

    case "RETURN_BOTTOM":
      return { ...withHistory(state, returnToDeckBottom(state.game, action.instanceId)), contextMenu: null };

    case "TAKE_SIDE":
      return { ...withHistory(state, takeSideCard(state.game, action.instanceId)), modal: null };
    ...
    case "ATTACH_ENERGY":
      try {
        return withHistory(state, attachEnergy(state.game, action.energyId, action.pokemonId));
      } catch { return state; }

    case "ATTACH_TOOL":
      try {
        return withHistory(state, attachTool(state.game, action.toolId, action.pokemonId));
      } catch { return state; }

    case "EVOLVE":
      try {
        return withHistory(state, evolve(state.game, action.evolutionId, action.targetId));
      } catch { return state; }

    case "DEVOLVE":
      try {
        return { ...withHistory(state, devolve(state.game, action.instanceId)), contextMenu: null };
      } catch { return state; }

    case "DAMAGE":
      return { ...withHistory(state, addDamage(state.game, action.instanceId, action.amount)), contextMenu: null };

    case "SET_DAMAGE":
      return { ...withHistory(state, setDamage(state.game, action.instanceId, action.value)), contextMenu: null };

    case "PLACE_STADIUM":
      return withHistory(state, placeStadium(state.game, action.instanceId));

    case "PROMOTE_TO_BATTLE":
      return { ...withHistory(state, promoteToBattle(state.game, action.instanceId)), modal: null };
...
function App() {
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      if (targetId.startsWith("card:")) {
        ...
        if (
          (inst.card.card_category === "基本エネルギー" || inst.card.card_category === "特殊エネルギー") &&
          targetInst.card.card_category === "ポケモン"
        ) {
          dispatch({ type: "ATTACH_ENERGY", energyId: instanceId, pokemonId: targetCardId });
          return;
        }
        ...
        if (
          ...
          dispatch({ type: "ATTACH_TOOL", toolId: instanceId, pokemonId: targetCardId });
          return;
        }
        ...
        if (
          ...
          dispatch({ type: "EVOLVE", evolutionId: instanceId, targetId: targetCardId });
          return;
        }
      ...
      if (ZONE_NAMES.includes(targetId as ZoneName)) {
        if (inst.card.card_category === "ポケモン") {
          dispatch({ type: "MOVE_WITH_CLEANUP", instanceId, from: fromZone, to: targetId as ZoneName });
        } else {
          dispatch({ type: "MOVE_CARD", instanceId, from: fromZone, to: targetId as ZoneName });
        }
      }
    },
    [game],
        ...
        <DraggableCard
          ...
          onClick={() =>
            dispatch({
              ...
              menu: { x: 100, y: 100, instanceId: id, zone },
            })
          }
        />
      );
  ...
  function contextMenuItems() {
    ...
    if (contextMenu.zone === "手札") {
      items.push({ label: "山札の上に戻す", action: () => dispatch({ type: "RETURN_TOP", instanceId: contextMenu.instanceId }) });
      items.push({ label: "山札の下に戻す", action: () => dispatch({ type: "RETURN_BOTTOM", instanceId: contextMenu.instanceId }) });
    }

    if ((contextMenu.zone === "バトル場" || contextMenu.zone === "ベンチ") && inst.card.card_category === "ポケモン") {
      items.push({ label: "ダメカン +10", action: () => dispatch({ type: "DAMAGE", instanceId: contextMenu.instanceId, amount: 10 }) });
      items.push({ label: "ダメカン -10", action: () => dispatch({ type: "DAMAGE", instanceId: contextMenu.instanceId, amount: -10 }) });
      items.push({
        label: "ダメカン値入力",
        action: () => {
          const val = prompt("ダメカン値を入力");
          if (val !== null) dispatch({ type: "SET_DAMAGE", instanceId: contextMenu.instanceId, value: parseInt(val, 10) || 0 });
        },
      });
      if (inst.evolutionStack.length > 0) {
        items.push({ label: "退化", action: () => dispatch({ type: "DEVOLVE", instanceId: contextMenu.instanceId }) });
      }
    }

    items.push({ label: "トラッシュへ", action: () => {
      dispatch({ type: "MOVE_WITH_CLEANUP", instanceId: contextMenu.instanceId, from: contextMenu.zone, to: "トラッシュ" });
    ...
    return (
      <div className="app">
        ...
        <div style={{ textAlign: "center", padding: "16px" }}>
          ...
          <button style={{ margin: "8px", padding: "12px 24px", background: "#3498db", color: "white", border: "none", borderRadius: "8px", cursor: "pointer", fontSize: "16px" }} onClick={() => dispatch({ type: "SETUP" })}>
            通常セットアップ
          </button>
        </div>
      </div>
  ...
  return (
    <div className="app">
      <DndProvider onDragEnd={handleDragEnd}>
        {warning && (
          <div className="warning" style={{ color: "#e74c3c", textAlign: "center", padding: "8px", cursor: "pointer" }} onClick={() => dispatch({ type: "CLEAR_WARNING" })}>
            {warning}
          </div>
        )}
      ...
      {modal === "search" && (
        <SearchModal
          cards={game.zones.山札.map((id) => game.cardInstances[id]!).filter(Boolean)}
          onSelect={(ids) => dispatch({ type: "SEARCH_SELECT", selectedIds: ids })}
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="search-cards">
              {game.zones.サイド.map((id) => {
                ...
                if (!inst) return null;
                return (
                  <div key={id} className="search-card" onClick={() => dispatch({ type: "TAKE_SIDE", instanceId: id })}>
                    <Card card={inst.card} />
                  </div>
                );
              })}
            ...
            <div className="modal-actions">
              <button onClick={() => dispatch({ type: "TAKE_SIDE_RANDOM" })}>ランダムに1枚取得</button>
              <button onClick={() => dispatch({ type: "SET_MODAL", modal: null })}>閉じる</button>
            </div>
          </div>
        </div>
      )}
                ...
                return (
                  <div key={id} className="search-card">
                    ...
                    <div style={{ display: "flex", gap: 2, marginTop: 2 }}>
                      <button style={{ fontSize: 8, padding: "2px 4px" }} onClick={() => dispatch({ type: "TRASH_RETURN", instanceId: id, to: "手札" })}>手札</button>
                      <button style={{ fontSize: 8, padding: "2px 4px" }} onClick={() => dispatch({ type: "TRASH_RETURN", instanceId: id, to: "山札上" })}>上</button>
                      <button style={{ fontSize: 8, padding: "2px 4px" }} onClick={() => dispatch({ type: "TRASH_RETURN", instanceId: id, to: "山札下" })}>下</button>
                    </div>
                  </div>
                );
              })}
      ...
      {showBenchSelector && (
        <BenchSelector
          benchPokemon={game.zones.ベンチ.map((id) => game.cardInstances[id]!).filter(Boolean)}
          onSelect={(id) => dispatch({ type: "PROMOTE_TO_BATTLE", instanceId: id })}
        />
      )}
    </div>
  );
